...
class PenjualanReport(models.TransientModel):
    _name = 'putramusik.penjualanreport'
    ...
    def action_penjualan_report(self):
        ...
        print(filter)
        penjualan = self.env['putramusik.penjualan'].search_read(filter)
        print(penjualan)
        data = {
            'form': self.read()[0],
        ...
        print(data)
        return self.env.ref('putramusik.wizzard_penjualanreport_pdf').report_action(self, data=data)